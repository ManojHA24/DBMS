...
SELECT M.MOVIE_TITLE FROM MOVIES M,DIRECTOR D WHERE M.DIR_ID=D.DIR_ID AND D.DIR_NAME="HITCHCOCK";
...
-- 6.Find the title of movies and number of stars for each movie that has at least one rating and find the highest number of stars that movie received. Sort the result by movie title.
SELECT M.MOVIE_TITLE,MAX(R.RATING_STARS) AS MAXIMUM_RATING,COUNT(*) AS NUMBER_OF_RATINGS FROM MOVIES M,RATING R 
WHERE M.MOVIE_ID=R.MOVIE_ID  GROUP BY R.MOVIE_ID HAVING COUNT(*)>0;


-- List actors acted in movies which released before 2000 and also sfter 2015.
